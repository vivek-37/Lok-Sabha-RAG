...
# --- CONFIGURATION ---
INPUT_CORPUS = "normalized_teammate_corpus.json"
OUTPUT_CORPUS = "chunked_rag_corpus_B.json"
...
def process_corpus():
    ...
    for doc in tqdm(records, desc="Chunking Outliers", unit="doc"):
        # --- THE NEW BYPASS VALVE ---
        # If the document is already a chunk from the adapter, skip it entirely!
        if "_part" in doc.get("id", ""):
